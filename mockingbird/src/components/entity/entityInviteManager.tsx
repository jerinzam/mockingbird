...
export const EntityInviteManager: React.FC<EntityInviteManagerProps> = ({ entityId, orgId }) => {
  ...
  useEffect(() => {
    async function fetchInvites() {
      try {
        const res = await fetch(`/api/dashboard/org/${orgId}/entities/${entityId}/invites`);
      ...
      <div className="bg-white border-2 border-black shadow-[4px_4px_0_#000] rounded-lg p-6">
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-gray-200 text-xs">
            {invites.map((invite) => {
              const inviteUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/org/${orgId}/entities/${entityId}?token=${invite.invite_code}`;
              return (
