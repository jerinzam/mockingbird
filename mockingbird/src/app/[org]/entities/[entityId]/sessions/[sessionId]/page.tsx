...
interface PageProps {
  params: Promise<{ entityId: string; sessionId: string; org: string }>;
}
...
//   const { entityId, sessionId, org } = use(params);
  useEffect(() => {
    ...
    params.then(p => setEntityId(p.entityId));
    params.then(p => setOrg(p.org));
  }, [params]);

  useEffect(() => {
    if (!org || !entityId || !sessionId) return;
      ...
      vapiInstanceRef.current.on('call-end', () => {
        ...
        setTimeout(() => {
          if (!isNavigatingRef.current) {
            ...
            router.push(`/entities/${entityId}/sessions/${sessionId}/review`);
          }
        }, 100);
      });
